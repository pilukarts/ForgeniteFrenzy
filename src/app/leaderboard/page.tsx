  ...
  const renderLeaderboardTable = (data: LeaderboardEntry[], title: string, icon: React.ReactNode) => {
    return (
        ...
        <CardContent className="p-0 sm:p-2"> 
            ...
            <TableHeader>
                <TableRow>
                <TableHead className="w-[40px] sm:w-[50px] text-center text-muted-foreground text-xs sm:text-sm">Rank</TableHead> 
                <TableHead className="text-muted-foreground text-xs sm:text-sm">Commander</TableHead> 
                <TableHead className="text-right text-muted-foreground text-xs sm:text-sm">Score</TableHead> 
                </TableRow>
            </TableHeader>
            <TableBody>
                {data.map((entry) => {
                ...
                return (
                    ...
                    <TableCell className="font-medium text-center text-sm sm:text-lg px-2 sm:px-4"> 
                    ...
                    <TableCell className="text-right font-mono text-primary text-xs sm:text-sm px-2 sm:px-4">{entry.score.toLocaleString()}</TableCell> 
                    </TableRow>
                );
                })}
                {data.length === 0 && (
      ...
      <div className="px-0 sm:px-4 pt-2 sm:pt-4"> 
        ...
        <Tabs defaultValue="global" className="w-full">
          <TabsList className="grid w-full grid-cols-1 mb-3 sm:mb-4 md:grid-cols-2 bg-background"> 
            <TabsTrigger value="global" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground text-sm sm:text-base">Global Rankings</TabsTrigger> 
          </TabsList>
