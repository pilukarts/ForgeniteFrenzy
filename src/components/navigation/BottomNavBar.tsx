...
const BottomNavBar: React.FC = () => {
  ...
  return (
    <nav className="absolute bottom-0 left-0 right-0 bg-background/80 backdrop-blur-sm border-t border-border/50 shadow-lg z-50 h-14 flex items-center md:hidden">
        ...
        <div className="flex w-max justify-around items-center px-1">
          {navItems.map(({ href, label, icon: Icon }) => {
            ...
            const linkContent = (
              ...
              >
                <Icon className={cn("h-5 w-5 mb-0.5", finalIsActive ? "text-primary" : "text-muted-foreground group-hover:text-foreground", label === 'Discord' ? 'fill-current' : '')} />
                {label}
              </div>
            );
